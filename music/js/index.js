...
$(document).ready(function () {
    init();
    function init(){
    	var song = ["The night we met","Say you will","Lặng thầm ","You to me, me to you"];
    	var artist = ["Lord Huron","Tokyo Square","Thuy Chi","Scenery of Riding Bicycle"];
        var current = 0;
        var audio = $('#audio');
        ...
        //audio[0].volume = .10;
        audio[0].play();
        playlist.on('click','.list_item', function(e){
            e.preventDefault();
            current = $(this).index();
            ...
            console.log("current")
			console.log(current)
            $('.mini-player').find(".title").text(song[current]);
            $('.mini-player').find(".artist").text( artist[current]);
            $('.playback_wrapper').find(".title").text(song[current]);
            $('.playback_wrapper').find(".artist").text(artist[current]);
